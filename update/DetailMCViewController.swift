...
class DetailMCViewController: BaseViewController {
    ...
    private func setView() {
        ...
        // Allow inventory labels to wrap and match styling of note labels
        inventoryLabel1.numberOfLines = 0
        inventoryLabel2.numberOfLines = 0
        inventoryLabel3.numberOfLines = 0
        inventoryLabel1.lineBreakMode = .byWordWrapping
        inventoryLabel2.lineBreakMode = .byWordWrapping
        inventoryLabel3.lineBreakMode = .byWordWrapping
        inventoryLabel1.font = UIFont.systemFont(ofSize: 14, weight: .regular)
        inventoryLabel2.font = UIFont.systemFont(ofSize: 14, weight: .regular)
        inventoryLabel3.font = UIFont.systemFont(ofSize: 14, weight: .regular)
    ...
    private func setDataView() {
        ...
        // Tính chiều cao bằng `systemLayoutSizeFitting` dựa trên Auto Layout
        self.view.layoutIfNeeded()

        // Prepare an array of the NCC container views
        var rowHeights: [CGFloat] = []
        let rows: [UIView] = [viewNCC1, viewNCC2, viewNCC3]

        for (index, row) in rows.enumerated() {
            if index < componentDetailModels.count && !row.isHidden {
                // Ensure the row has correct layout before measuring
                row.setNeedsLayout()
                row.layoutIfNeeded()

                // Use systemLayoutSizeFitting with compressed size to get intrinsic height
                let targetSize = CGSize(width: row.bounds.width, height: UIView.layoutFittingCompressedSize.height)
                let fittingSize = row.systemLayoutSizeFitting(targetSize,
                                                              withHorizontalFittingPriority: .required,
                                                              verticalFittingPriority: .fittingSizeLevel)
                // Add a small padding to avoid clipping
                rowHeights.append(ceil(fittingSize.height) + 8)
            } else {
                rowHeights.append(0)
            }
        }

        // Determine max row height among visible rows (ensure a minimum)
        let visibleHeights = rowHeights.filter { $0 > 0 }
        let maxRowHeight = (visibleHeights.max() ?? 60)

        // cstHeightViewSL holds total height for rows area; add extra padding (e.g., header/footer)
        cstHeightViewSL.constant = CGFloat(componentDetailModels.count) * maxRowHeight + 60
    }
    ...
    override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        ...
        if traitCollection.userInterfaceIdiom == .phone && previousTraitCollection?.userInterfaceIdiom == .phone && traitCollection.horizontalSizeClass != previousTraitCollection?.horizontalSizeClass {
            ...
            numberItemsTextfield.text = numberFormatter.string(from: numberFormatter.number(from: numberItemsTextfield.text ??  "") ?? NSNumber(value: 0))
            numberItemsTextfield2.text = numberFormatter.string(from: numberFormatter.number(from: numberItemsTextfield2.text ??  "") ?? NSNumber(value: 0))
            numberItemsTextfield3.text = numberFormatter.string(from: numberFormatter.number(from: numberItemsTextfield3.text ??  "") ?? NSNumber(value: 0))
        }
    }
